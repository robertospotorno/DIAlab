import { normalizeData, basicEIS, calculateDIA, buildSpectra, spectraToPairedColumns, processSlope, reconstructPoint, identifyProcesses, summarizeProcess } from './core.js';
...
const processColors = ['#7f9fb7','#8fa98a','#c5a27f','#9d91b5','#b88c9b','#79aaa5','#b5a06f','#8e9ca8'];
...
function activateUI() {
  ['exportButton','recalculateButton','rangeHigh','rangeLow','pointIndex','previewCorrection','identifyProcessesButton'].forEach(id => $(id).disabled=false);
...
function processOptions() {
  return {
    ...currentOptions(),
    processPeakThreshold: Number($('processPeakThreshold')?.value ?? 0.15),
    processSmoothRadius: 4,
    processMinSeparation: 0.35,
    maxProcesses: 6
  };
}
...
function autoIdentifyProcesses() {
  if(!state.dia) return;
  state.processDetection=identifyProcesses(state.dia,processOptions());
  state.selectedProcess=Math.min(state.selectedProcess,Math.max(0,state.processDetection.summaries.length-1));
  renderProcessEditor();
}

function refreshProcessSummaries() {
  if(!state.dia || !state.processDetection?.bounds?.length) return;
  const b=state.processDetection.bounds;
  state.processDetection.summaries=[];
  for(let i=0;i<b.length-1;i++) state.processDetection.summaries.push(summarizeProcess(state.dia,b[i],b[i+1],processOptions()));
  renderProcessEditor();
}

function processSummaryRows() {
  return state.processDetection?.summaries || [];
}

function selectProcess(index) {
  const n=processSummaryRows().length;
  if(!n) return;
  state.selectedProcess=Math.max(0,Math.min(n-1,index));
  renderProcessEditor();
  renderProcessContext();
}

function processFrequencyRange(summary) {
  return `${fmt(summary.fHigh,4)} → ${fmt(summary.fLow,4)}`;
}
...
function renderProcessEditor() {
  ...
  if(!tbody) return;
  tbody.innerHTML='';
  const rows=processSummaryRows();
  $('identifiedCount').textContent=rows.length;
  rows.forEach((s,i)=>{
    const tr=document.createElement('tr');
    if(i===state.selectedProcess) tr.classList.add('active-process');
    tr.innerHTML=`<td><span class="process-chip"><span class="process-swatch" style="background:${processColors[i%processColors.length]}"></span>P${i+1}</span></td><td>${processFrequencyRange(s)}</td><td>${fmt(s.R,5)}</td><td>${fmt(s.C,5)}</td><td>${fmt(s.Ceff,5)}</td><td>${fmt(s.T,5)}</td><td>${fmt(s.alphaPeak,5)}</td><td>${fmt(s.alphaMedian,5)}</td>`;
    tr.addEventListener('click',()=>selectProcess(i));
    tbody.appendChild(tr);
  });
  updateProcessBoundaryControls();
}

function updateProcessBoundaryControls() {
  const rows=processSummaryRows();
  const hf=$('processHF'), lf=$('processLF');
  if(!rows.length || !state.processDetection?.bounds) {
    if(hf) hf.disabled=true; if(lf) lf.disabled=true;
    $('selectedProcessLabel').textContent='—'; $('processHFLabel').textContent='—'; $('processLFLabel').textContent='—';
    return;
  }
  const i=Math.max(0,Math.min(rows.length-1,state.selectedProcess));
  const b=state.processDetection.bounds;
  const max=state.dia.primary.nu.length-1;
  const minWidth=Math.max(3,Math.round(max*0.01));
  hf.disabled=false; lf.disabled=false;
  hf.min=i===0?0:b[i-1]+minWidth;
  hf.max=b[i+1]-minWidth;
  hf.value=b[i];
  lf.min=b[i]+minWidth;
  lf.max=i===rows.length-1?max:b[i+2]-minWidth;
  lf.value=b[i+1];
  $('selectedProcessLabel').textContent=`Process ${i+1}`;
  $('processHFLabel').textContent=`${fmt(state.dia.primary.frequency[b[i]],5)} Hz`;
  $('processLFLabel').textContent=`${fmt(state.dia.primary.frequency[b[i+1]],5)} Hz`;
}

function editProcessBoundary(which) {
  if(!state.processDetection?.bounds?.length) return;
  const i=state.selectedProcess,b=state.processDetection.bounds;
  if(which==='hf') b[i]=Number($('processHF').value);
  else b[i+1]=Number($('processLF').value);
...
function processBandShapesForBode() {
  const shapes=[];
  processSummaryRows().forEach((s,i)=>{
    const c=processColors[i%processColors.length];
    shapes.push({type:'rect',xref:'x',yref:'paper',x0:s.fLow,x1:s.fHigh,y0:0,y1:1,fillcolor:hexToRgba(c,i===state.selectedProcess?.18:.10),line:{width:0},layer:'below'});
  });
  return shapes;
}

function processBandAnnotationsBode() {
  return processSummaryRows().map((s,i)=>({
    xref:'x',yref:'paper',x:Math.sqrt(s.fHigh*s.fLow),y:.98,text:`P${i+1}`,showarrow:false,
    font:{size:10,color:processColors[i%processColors.length]},bgcolor:'rgba(255,255,255,.72)',borderpad:2
  }));
}

function processBandShapesTemporal() {
  const p=state.dia.primary, shapes=[];
  processSummaryRows().forEach((s,i)=>{
    const c=processColors[i%processColors.length];
    shapes.push({type:'rect',xref:'x',yref:'paper',x0:p.nu[s.startIndex],x1:p.nu[s.endIndex],y0:0,y1:1,fillcolor:hexToRgba(c,i===state.selectedProcess?.18:.10),line:{width:0},layer:'below'});
    shapes.push({type:'line',xref:'x',yref:'paper',x0:p.nu[s.startIndex],x1:p.nu[s.startIndex],y0:0,y1:1,line:{color:hexToRgba(c,.55),width:i===state.selectedProcess?2:1,dash:'dot'}});
    shapes.push({type:'line',xref:'x',yref:'paper',x0:p.nu[s.endIndex],x1:p.nu[s.endIndex],y0:0,y1:1,line:{color:hexToRgba(c,.55),width:i===state.selectedProcess?2:1,dash:'dot'}});
  });
  return shapes;
}

function processBandAnnotationsTemporal() {
  const p=state.dia.primary;
  return processSummaryRows().map((s,i)=>({
    xref:'x',yref:'paper',x:(p.nu[s.startIndex]+p.nu[s.endIndex])/2,y:.98,text:`P${i+1}`,showarrow:false,
    font:{size:10,color:processColors[i%processColors.length]},bgcolor:'rgba(255,255,255,.72)',borderpad:2
  }));
}

function processBandTracesNyquist() {
  const traces=[];
  processSummaryRows().forEach((s,i)=>{
    const ids=[];
    state.data.frequency.forEach((f,j)=>{ if(f<=s.fHigh && f>=s.fLow) ids.push(j); });
    if(ids.length>1) traces.push({
      x:ids.map(j=>state.data.re[j]),y:ids.map(j=>-state.data.im[j]),mode:'lines',name:`Process ${i+1}`,
      line:{color:processColors[i%processColors.length],width:i===state.selectedProcess?11:8},opacity:i===state.selectedProcess?.35:.22,
      hoverinfo:'skip',showlegend:false
    });
    const h=nearestFrequencyIndex(s.fHigh),l=nearestFrequencyIndex(s.fLow);
    traces.push({
      x:[state.data.re[h],state.data.re[l]],y:[-state.data.im[h],-state.data.im[l]],mode:'markers',showlegend:false,
      marker:{size:i===state.selectedProcess?8:6,color:processColors[i%processColors.length],symbol:['triangle-up','triangle-down'],line:{width:1,color:'#fff'}},
      text:[`P${i+1} HF`,`P${i+1} LF`],customdata:[[s.fHigh],[s.fLow]],hovertemplate:'%{text}<br>f=%{customdata[0]:.4g} Hz<extra></extra>'
    });
  });
  return traces;
}

function renderProcessNyquist() {
  if(!state.data) return;
  const traces=[{
    x:state.data.re,y:state.data.im.map(v=>-v),mode:'lines+markers',name:'Data',
    line:{color:colors.data,width:1.8},marker:{size:4,color:colors.data},
    customdata:state.data.frequency.map((f,i)=>[f,state.data.re[i],state.data.im[i]]),
    hovertemplate:'f=%{customdata[0]:.4g} Hz<br>Z′=%{customdata[1]:.5g} Ω<br>Z″=%{customdata[2]:.5g} Ω<br>−Z″=%{y:.5g} Ω<extra></extra>'
  },...processBandTracesNyquist(),...correctionTracesNyquist()];
  const l=baseLayout('Nyquist — identified processes','Z′ [Ω]','−Z″ [Ω]');
  l.yaxis.scaleanchor='x'; l.yaxis.scaleratio=1; l.annotations=decadeAnnotations();
  Plotly.react('processNyquistPlot',traces,l,plotConfig);
}

function renderProcessBode() {
  if(!state.data) return;
  const basic=basicEIS(state.data);
  const traces=[
    {x:state.data.frequency,y:basic.magnitude,mode:'lines+markers',name:'|Z|',line:{color:'#556f84'},marker:{size:4},yaxis:'y'},
    {x:state.data.frequency,y:basic.phase,mode:'lines+markers',name:'Phase',line:{color:'#8b7163'},marker:{size:4},yaxis:'y2'},
    ...correctionTracesBode()
  ];
  const l=baseLayout('Bode — identified processes','Frequency [Hz]','|Z| [Ω]');
  l.xaxis.type='log';l.xaxis.autorange='reversed';l.yaxis.type='log';
  l.yaxis2={title:'Phase [°]',overlaying:'y',side:'right',gridcolor:'rgba(0,0,0,0)'};l.margin.r=65;
  l.shapes=processBandShapesForBode(); l.annotations=processBandAnnotationsBode();
  Plotly.react('processBodePlot',traces,l,plotConfig);
}

function renderProcessTemporal() {
  if(!state.dia) return;
  const p=state.dia.primary,traces=[];
  if(visible('T')) traces.push({x:p.nu,y:p.logT,name:'log T',mode:'lines',line:{color:colors.T,width:2}});
  if(visible('R')) traces.push({x:p.nu,y:p.logR,name:'log R',mode:'lines',line:{color:colors.R,width:2}});
  if(visible('C')) traces.push({x:p.nu,y:p.logC,name:'log C',mode:'lines',line:{color:colors.C,width:2}});
  if(visible('Rs')) traces.push({x:p.nu,y:p.logRs,name:'log Rs',mode:'lines',line:{color:colors.Rs,width:2}});
  if(visible('Ceff')) traces.push({x:state.dia.secondary.nu,y:state.dia.secondary.logCeff,name:'log Ceff',mode:'lines',line:{color:colors.Ceff,width:1.6}});
  const l=baseLayout('Primary DIA — identified process bands','ν = log₁₀(1/ω)','log₁₀(P)');
  l.shapes=processBandShapesTemporal(); l.annotations=processBandAnnotationsTemporal();
  Plotly.react('processTemporal',traces,l,plotConfig);
}

function renderProcessContext() {
  if(!state.processDetection) return;
  renderProcessNyquist();
  renderProcessBode();
  renderProcessTemporal();
  renderProcessEditor();
}
...
function exportWorkbook() {
  ...
  if(state.processDetection?.summaries?.length) {
    const pr=[['Process','f_high_Hz','f_low_Hz','R_Ohm','C_F','Ceff_F','T_s','alpha_peak','alpha_median','start_index','end_index']];
    state.processDetection.summaries.forEach((q,i)=>pr.push([i+1,q.fHigh,q.fLow,q.R,q.C,q.Ceff,q.T,q.alphaPeak,q.alphaMedian,q.startIndex,q.endIndex]));
    XLSX.utils.book_append_sheet(wb,XLSX.utils.aoa_to_sheet(pr),'Processes');
  }
...
$('showReferences').addEventListener('change',()=>{ if(state.data){ renderEISContexts(); renderProcessContext(); } });
...
$('identifyProcessesButton').addEventListener('click',()=>{ if(state.dia){ autoIdentifyProcesses(); renderProcessContext(); setStatus(`${processSummaryRows().length} process(es) identified from the T spectrum.`, 'success'); } });
$('processPeakThreshold').addEventListener('change',()=>{ if(state.dia){ autoIdentifyProcesses(); renderProcessContext(); } });

let processBoundaryFrame=null;
function scheduleProcessBoundaryEdit(which) {
  if(!state.processDetection) return;
  if(processBoundaryFrame) cancelAnimationFrame(processBoundaryFrame);
  processBoundaryFrame=requestAnimationFrame(()=>{ editProcessBoundary(which); processBoundaryFrame=null; });
}
$('processHF').addEventListener('input',()=>scheduleProcessBoundaryEdit('hf'));
$('processLF').addEventListener('input',()=>scheduleProcessBoundaryEdit('lf'));

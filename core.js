...
export function normalizeData(rows) {
  const parsed = [];
  let imaginarySignAdjusted = 0;
  for (const row of rows) {
    if (!row || row.length < 3) continue;
    const f = Number(row[0]), re = Number(row[1]), imRaw = Number(row[2]);
    if (f > 0 && Number.isFinite(f) && Number.isFinite(re) && Number.isFinite(imRaw)) {
      // DIALab uses the capacitive EIS convention Z'' <= 0 so that Nyquist is plotted as -Z'' >= 0.
      // Files exported with +Im(Z) are therefore normalized automatically to the same convention.
      const im = -Math.abs(imRaw);
      if (imRaw > 0) imaginarySignAdjusted += 1;
      parsed.push({ f, re, im });
    }
  }
  ...
  return {
    ...
    normalization: {
      imaginarySignAdjusted,
      nyquistConvention: "Zimag = -abs(Zimag input); plot ordinate = -Zimag",
    },
  };
}
...
export function spectraToPairedColumns(spectra) {
  const series=[
    ['Primary T',spectra.primary.T],
    ['Primary Rs',spectra.primary.Rs],
    ['Primary R',spectra.primary.R],
    ['Primary C',spectra.primary.C],
    ['Secondary T',spectra.secondary.T],
    ['Secondary n',spectra.secondary.n],
    ['Secondary 1-n',spectra.secondary.oneMinusN],
    ['Secondary log Ceff',spectra.secondary.Ceff]
  ];
  const headers=[];
  series.forEach(([name])=>headers.push(`${name} parameter`,`${name} count`));
  const rows=[headers];
  const maxRows=Math.max(...series.map(([,o])=>o?.x?.length||0));
  for(let i=0;i<maxRows;i++) {
    const row=[];
    series.forEach(([,o])=>row.push(o?.x?.[i] ?? '',o?.y?.[i] ?? ''));
    rows.push(row);
  }
  return {rows, seriesCount:series.length};
}
...
export function reconstructPoint(data, index, useHigher = false) {
  ...
  // Reference imaginary components are treated as positive Nyquist ordinates (-Z'').
  // abs() keeps the reconstruction robust even if reconstructPoint is called on non-normalized data.
  const zd1 = Math.abs(data.im[idxH]), zd2 = Math.abs(data.im[idxL]);
  ...
  // With the equations ported from MATLAB this expression has the sign of Z'' itself.
  // The previous web version inverted it a second time, sending the preview to the opposite Nyquist quadrant.
  const zppRaw = omega3*R1*tau1/(omega3*omega3*tau1*tau1+1) + omega3*R2*tau2/(omega3*omega3*tau2*tau2+1);
  if (![zp,zppRaw].every(Number.isFinite)) throw new Error('Reconstructed point is not finite.');
  const zpp = -Math.abs(zppRaw); // enforce the same capacitive convention used at import
  return { index, idxH, idxL, re: zp, im: zpp, minusIm: -zpp, parameters: {T,C,Cinf,sigma0,tau1,tau2,R1,R2} };
}
...
function movingAverage(values, radius = 3) {
  return values.map((_, i) => {
    ...
    for (let j = Math.max(0, i - radius); j <= Math.min(values.length - 1, i + radius); j++) {
      if (Number.isFinite(values[j])) { sum += values[j]; n += 1; }
    }
    return n ? sum / n : NaN;
  });
}

function peakX(spec, predicate = null) {
  if (!spec?.x?.length) return NaN;
  let best = -1, bestY = -Infinity;
  for (let i = 0; i < spec.x.length; i++) {
    const x = spec.x[i], y = spec.y[i];
    if (!Number.isFinite(x) || !Number.isFinite(y) || (predicate && !predicate(x))) continue;
    if (y > bestY) { bestY = y; best = i; }
  }
  return best >= 0 ? spec.x[best] : NaN;
}
...
export function summarizeProcess(dia, startIndex, endIndex, options = {}) {
  const maxP = dia.primary.nu.length - 1;
  const lo = Math.max(0, Math.min(maxP, Math.min(startIndex, endIndex)));
  const hi = Math.max(lo, Math.min(maxP, Math.max(startIndex, endIndex)));
  const spectra = buildSpectra(dia, lo, hi, options);
  const logT = peakX(spectra.primary.T);
  const logR = peakX(spectra.primary.R);
  const logC = peakX(spectra.primary.C);

  const sHi = Math.min(hi, dia.secondary.nu.length - 1);
  const sLo = Math.min(lo, sHi);
  const alphaValues = [], alphaNu = [];
  for (let i = sLo; i <= sHi; i++) {
    const a = dia.secondary.n[i], nu = dia.secondary.nu[i];
    if (Number.isFinite(a) && Number.isFinite(nu)) { alphaValues.push(a); alphaNu.push(nu); }
  }
  const alphaSpec = weightedSpectrum(alphaValues, alphaNu, Math.max(0, Math.min(5, Math.round(options.roundSecondary ?? 3))), Math.max(0.000001, Number(options.selectivity ?? 0.01)));
  const alphaPeak = peakX(alphaSpec);
  const alphaMedian = medianFinite(alphaValues);

  const ceffLogs = [], ceffNu = [];
  for (let i = sLo; i <= sHi; i++) {
    const c = dia.secondary.Ceff[i], nu = dia.secondary.nu[i];
    if (Number.isFinite(c) && c > 1e-24 && Number.isFinite(nu)) {
      ceffLogs.push(Math.log10(c)); ceffNu.push(nu);
    }
  }
  const ceffSpec = weightedSpectrum(ceffLogs, ceffNu, Math.max(0, Math.min(5, Math.round(options.roundSecondary ?? 3))), Math.max(0.000001, Number(options.selectivity ?? 0.01)));
  const logCeff = peakX(ceffSpec, x => x > -24);

  const fromLog = x => Number.isFinite(x) ? 10 ** x : NaN;
  return {
    startIndex: lo,
    endIndex: hi,
    fHigh: dia.primary.frequency[lo],
    fLow: dia.primary.frequency[hi],
    logT, logR, logC, logCeff,
    T: fromLog(logT),
    R: fromLog(logR),
    C: fromLog(logC),
    Ceff: fromLog(logCeff),
    alphaPeak,
    alphaMedian,
    spectra: { ...spectra, alpha: alphaSpec, CeffClean: ceffSpec }
  };
}

export function identifyProcesses(dia, options = {}) {
  const maxP = dia.primary.nu.length - 1;
  const spectra = buildSpectra(dia, 0, maxP, options);
  const spec = spectra.primary.T;
  if (!spec.x.length) return { peaks: [], centers: [], bounds: [0, maxP], summaries: [summarizeProcess(dia, 0, maxP, options)] };

  const radius = Math.max(1, Math.round(options.processSmoothRadius ?? 4));
  const ys = movingAverage(spec.y, radius);
  const maxY = Math.max(...ys.filter(Number.isFinite));
  const threshold = Math.max(0.02, Math.min(0.95, Number(options.processPeakThreshold ?? 0.15)));
  const minSeparation = Math.max(0.05, Number(options.processMinSeparation ?? 0.35));
  const maxProcesses = Math.max(1, Math.min(8, Math.round(options.maxProcesses ?? 6)));
  const candidates = [];

  for (let i = 0; i < ys.length; i++) {
    const y = ys[i];
    if (!Number.isFinite(y) || y < maxY * threshold) continue;
    const left = i === 0 ? -Infinity : ys[i - 1];
    const right = i === ys.length - 1 ? -Infinity : ys[i + 1];
    if (y >= left && y >= right && (y > left || y > right)) {
      candidates.push({ x: spec.x[i], y, rawY: spec.y[i], spectrumIndex: i });
    }
  }
  if (!candidates.length) {
    let best = 0;
    for (let i = 1; i < ys.length; i++) if (ys[i] > ys[best]) best = i;
    candidates.push({ x: spec.x[best], y: ys[best], rawY: spec.y[best], spectrumIndex: best });
  }

  candidates.sort((a, b) => b.y - a.y);
  const selected = [];
  for (const c of candidates) {
    if (selected.every(p => Math.abs(p.x - c.x) >= minSeparation)) selected.push(c);
    if (selected.length >= maxProcesses) break;
  }
  selected.sort((a, b) => a.x - b.x); // log(T) ascending = high-frequency to low-frequency processes

  const centers = selected.map(p => {
    let idx = 0, best = Infinity;
    for (let i = 0; i < dia.primary.nu.length; i++) {
      const d = Math.abs(dia.primary.nu[i] - p.x);
      if (d < best) { best = d; idx = i; }
    }
    return idx;
  });

  // Collapse accidental duplicate centers after mapping to the densified nu grid.
  const uniqueCenters = [];
  const uniquePeaks = [];
  centers.forEach((c, i) => {
    if (!uniqueCenters.length || c !== uniqueCenters[uniqueCenters.length - 1]) {
      uniqueCenters.push(c); uniquePeaks.push(selected[i]);
    }
  });

  const bounds = [0];
  for (let i = 0; i < uniqueCenters.length - 1; i++) bounds.push(Math.round((uniqueCenters[i] + uniqueCenters[i + 1]) / 2));
  bounds.push(maxP);
  const summaries = uniqueCenters.map((_, i) => summarizeProcess(dia, bounds[i], bounds[i + 1], options));
  return { peaks: uniquePeaks, centers: uniqueCenters, bounds, summaries, detectionSpectrum: spec, threshold };
}
